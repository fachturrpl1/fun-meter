...
export const createLazyRoute = <P extends Record<string, unknown> = Record<string, unknown>>(
  ...
  const LazyRouteComponent = (props: P) => (
...
export const LazyModal = ({ isOpen, children }: LazyModalProps) => {
...
export const Debounced = ({ children, delay = 300, dependencies = [] }: DebouncedProps) => {
  ...
  const timeoutRef = useRef<NodeJS.Timeout | undefined>(undefined); // ✅ DIPERBAIKI
...
const MemoizedWrapperComponent = React.memo(({ 
  children
}: {
  children: ReactNode;
  dependencies?: unknown[];
...
MemoizedWrapperComponent.displayName = 'MemoizedWrapper';
export const MemoizedWrapper = MemoizedWrapperComponent;
...
export default LazyComponentsExports;
