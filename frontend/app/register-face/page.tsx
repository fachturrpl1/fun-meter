...
// import { request } from "@/lib/api"; // Available for future API calls
...
function RegisterFacePageContent() {
  ...
  // Face detection state (available for future face detection features)
  // const [faceDetected, setFaceDetected] = useState(false);
  const [facesCount, setFacesCount] = useState(0);
  const [uploadFile, setUploadFile] = useState<File | null>(null);
  const [previewSrc, setPreviewSrc] = useState<string>("");
  const [uploadPreviewSrc, setUploadPreviewSrc] = useState<string>("");
  // Freeze state for camera (available for future freeze functionality)
  // const [freeze, setFreeze] = useState(false);
  const [previewLoading] = useState(false);
  // Preview token for face detection (available for future preview features)
  // const [previewToken, setPreviewToken] = useState<string>("");
  // Preview faces for detection overlay (available for future face overlay)
  ...
  // Settings (available for future camera configuration)
  ...
  const socket = useWs({
    ...
    on: {
      ...
      face_detection(...args: unknown[]) {
        ...
        // setFaceDetected(faces.length > 0); // Commented out as faceDetected state is not used
  ...
  const capturePhoto = async () => {
    try {
      ...
      // setFreeze(true); // Commented out as freeze state is not used
      ...
      try {
        ...
        // setPreviewFaces(faces); // Commented out as previewFaces state is not used
        // setPreviewToken(String(resp?.token || "")); // Commented out as previewToken state is not used
        ...
        // setPreviewToken(""); // Commented out as previewToken state is not used
        // setPreviewFaces([]); // Commented out as previewFaces state is not used
        setFacesCount(0);
      }
      ...
      // setFreeze(false); // Commented out as freeze state is not used
      try { await videoRef.current?.play(); } catch {}
    } finally {
      // setFreeze(false); // Commented out as freeze state is not used
    }
  ...
  const buildImageData = async (): Promise<string | null> => {
    ...
    canvas.height = videoRef.current.videoHeight || 480;
    ctx.drawImage(videoRef.current, 0, 0);
    const jpegQ = Number((jpegQuality as { model?: number })?.model ?? 0.8);
