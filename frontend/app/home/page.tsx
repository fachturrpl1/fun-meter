...
"use client";
// import { useRouter } from "next/navigation"; // Available for future navigation
...
export default function HomePage() {
  ...
  // Router for navigation (available for future use)
  ...
  // Attendance results (currently using lastAttResults instead)
  const [, setAttendanceResults] = useState<Array<{ bbox?: number[]; box?: number[]; label?: string; name?: string }>>([]);
  ...
  // Marquee refs (available for future marquee animation)
  ...
  const [repeatCount] = useState(3); // Jumlah repeat iklan ke kanan dan kiri (total = 1 center + 3 kiri + 3 kanan = 7)
  // setRepeatCount available for dynamic repeat count adjustment
  ...
  const drawBoxWithLabels = (x: number, y: number, w: number, h: number, name: string, expr: string, color: string) => {
    ...
    if (!hostRect) return;
    const padX = 10, th = 24, gap = 6;
    // padY available for future vertical padding adjustments
    // const padY = 4;
